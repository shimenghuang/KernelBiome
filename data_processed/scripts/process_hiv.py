...
# Paths
data_path_in = "data_original/sCD14_HIV"
...
def process_data(data_path_in, data_path_out):
    ...
    data_abs = pd.read_csv(join(data_path_in, "hiv_X.csv"))
    data_abs.set_index('Unnamed: 0', inplace=True)  # needed for group_taxa
    y_df = pd.read_csv(join(data_path_in, "hiv_y.csv"))
    ...
    data_df.to_csv(join(data_path_out, "hiv.csv"), index=False)
