...
# Paths
data_path_in = "data_original/Tara"
...
def process_data(data_path_in, data_path_out):
    ...
    data_abs = pd.read_csv(join(data_path_in, "tara_X.csv"))
    data_abs.set_index('Unnamed: 0', inplace=True)
    y_df = pd.read_csv(join(data_path_in, "tara_y.csv"))
    ...
    data_df.to_csv(join(data_path_out, "tara.csv"), index=False)
