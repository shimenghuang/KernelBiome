...
from sklearn.model_selection import GridSearchCV, KFold, StratifiedKFold, LeaveOneGroupOut
...
def get_kr_penalty(X, kmat_fun, alpha_max=5.0, n_grid=5, name=None, verbose=False):
    ...
    if np.isnan(K).any() or np.isinf(K).any() or (K == 0.0).all():
        ...
        # Note: we might need to add something strictly greater than rank_tol?
...
def run_nested_cv_(X_orig, y, cv_split, estimator, estimator_param_grid, scoring, kmat_fun=None, center_kmat=False,
                   use_count=False, eps=0, n_fold_outer=10, n_fold_inner=5, n_jobs=6, verbose=0):
    """
    General function for nested CV calls to allow the outer CV being an arbitrary data splitter.

    cv_split: e.g. logo.split(X, y, grp), need to make sure the dimensions of X is the same as X_orig and also y.
    """
    ...
    for kk, (train_index, test_index) in enumerate(cv_split):
        ...
        try:
            ...
            # TODO: catch and print the error instead
            warnings.warn(
...
def run_nested_cv_kfold(X_orig, y, estimator, estimator_param_grid, scoring, kmat_fun=None, center_kmat=False, use_count=False, eps=0,
    ...
    return run_nested_cv_(X_orig, y, kf.split(X_orig, y), estimator, estimator_param_grid, scoring, kmat_fun, center_kmat,
                          use_count, eps, n_fold_outer, n_fold_inner, n_jobs, verbose)


def run_nested_cv_logo(X_orig, y, grp, estimator, estimator_param_grid, scoring, kmat_fun=None, center_kmat=False, use_count=False, eps=0,
                       n_fold_inner=5, n_jobs=6, verbose=0):
    logo = LeaveOneGroupOut()
    n_fold_outer = logo.get_n_splits(X_orig, y, grp)
    return run_nested_cv_(X_orig, y, logo.split(X_orig, y, grp), estimator, estimator_param_grid, scoring, kmat_fun, center_kmat,
                          use_count, eps, n_fold_outer, n_fold_inner, n_jobs, verbose)


def run_experiments(X, y, mod_with_params, param_grid_ke, param_grid_rf=None, param_grid_bl=None, center_kmat=False,
                    outer_cv_type="kfold", grp=None, n_fold_outer=10, n_fold_inner=5, type='regression', scoring='neg_mean_squared_error', kernel_estimator='kr',
    ...
    Note: for classification, the kfold CV is stratified.
    """
    ...
    if type == 'regression':
        ...
        def Dummy(): return DummyClassifier(random_state=random_state)
        KE = svm.SVC
        stratify = True
    else:
        raise ValueError(
    ...
    for ii, (name, fun) in enumerate(mod_with_params.items()):
        ...
        # function args by model names
        if name == 'RF use comp':
            estimator = RF(max_depth=np.sqrt(
                X.shape[0]), random_state=random_state)
            kmat_fun = None
            use_count = False
            ...
            kmat_fun = fun
            use_count = False
            param_grid = param_grid_ke_use

        # call different nested CV according to CV type
        if outer_cv_type == "kfold":
            if param_grid is not None:
                time_estimators[ii] = timeit.default_timer()
                train_scores_all[ii, :], test_scores_all[ii, :], selected_params_all[ii, :] = run_nested_cv_kfold(
                    X, y, estimator, param_grid, scoring, kmat_fun=kmat_fun, center_kmat=center_kmat, use_count=use_count, eps=0,
                    n_fold_outer=n_fold_outer, n_fold_inner=n_fold_inner, stratify=stratify, shuffle=False, random_state=random_state,
                    n_jobs=n_jobs, verbose=verbose)
                time_estimators[ii] = timeit.default_timer() - \
                    time_estimators[ii]
            else:
                print(f"`param_grid_ke_use` is None. Run skipped.")
        elif outer_cv_type == "logo":
            if param_grid is not None:
                time_estimators[ii] = timeit.default_timer()
                train_scores_all[ii, :], test_scores_all[ii, :], selected_params_all[ii, :] = run_nested_cv_logo(
                    X, y, grp, estimator, param_grid, scoring, kmat_fun=kmat_fun, center_kmat=center_kmat, use_count=use_count, eps=0,
                    n_fold_inner=n_fold_inner, n_jobs=n_jobs, verbose=verbose)
                time_estimators[ii] = timeit.default_timer() - \
                    time_estimators[ii]
            else:
                print(f"`param_grid_ke_use` is None. Run skipped.")
        else:
            print(f"{outer_cv_type} not implemented.")
