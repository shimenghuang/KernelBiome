...
def run_nested_cv_(X_orig, y, cv_split, estimator, estimator_param_grid, scoring, kmat_fun=None, center_kmat=False,
    ...
    cv_split: e.g. logo.split(X, y, grp)
    """
...
def run_nested_cv_skfold(X_orig, y, estimator, estimator_param_grid, scoring, kmat_fun=None, center_kmat=False, use_count=False, eps=0,
...
def run_experiments(X, y, mod_with_params, param_grid_ke, param_grid_rf=None, param_grid_bl=None, center_kmat=False,
                    outer_cv_type="skfold", grp=None, n_fold_outer=10, n_fold_inner=5, type='regression', scoring='neg_mean_squared_error', kernel_estimator='kr',
    ...
    for ii, (name, fun) in enumerate(mod_with_params.items()):
        ...
        if outer_cv_type == "skfold":
            if param_grid is not None:
                time_estimators[ii] = timeit.default_timer()
                train_scores_all[ii, :], test_scores_all[ii, :], selected_params_all[ii, :] = run_nested_cv_skfold(
