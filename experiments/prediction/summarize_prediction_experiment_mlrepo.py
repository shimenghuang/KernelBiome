...
grid_dim_x_roc = 4
grid_dim_y_roc = 5
datasets = [dd.replace('.csv', '')
            for dd in os.listdir(data_path)
            if 'mlr_' in dd]
reg_ds = ['mlr_ravel_ph', 'mlr_gevers_pcdai-rectum', 'mlr_gevers_pcdai-ileum',
          'mlr_yatsunenko_baby-age', 'mlr_ravel_nugent-score']
for ds in reg_ds:
    datasets.remove(ds)
datasets += reg_ds
...
def plot_roc_curves(ax, yscores, ylab, xlab):
    ...
    data_name_short = data_name.removeprefix('mlr_')
    if data_name == 'mlr_hmp_sub-supragingivalplaque-paired':
        ax.set_title('hmp_sub-supra-paired', pad=3.0)
    else:
        ax.set_title(data_name_short, pad=3.0)
    # further adjustments
...
fig.legend(handles=legend_handle,
           ...
           labelspacing=0.1,
           columnspacing=0.5,
           bbox_to_anchor=(0.51, -0.03),  # manual adjust
...
fig_roc.set_size_inches(width, width)
